...
st.title("⚡ Pivot Compare — Fast Baseline")

c1, c2 = st.columns(2)
with c1: f1 = st.file_uploader("Pivot A (CSV)", type=["csv"], key="a")
with c2: f2 = st.file_uploader("Pivot B (CSV)", type=["csv"], key="b")
...
if f1 and f2:
    ...
    if not shared:
        st.error("No shared columns.")
    else:
        keys_guess = [c for c in shared if (df1[c].dtype=='object') or (df2[c].dtype=='object')]
        keys = st.multiselect("Key columns", shared, default=keys_guess[:3])
        if keys:
