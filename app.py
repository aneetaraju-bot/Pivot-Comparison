...
st.caption(
    "Upload two pivot CSVs with similar headings (single pair) or many CSVs (batch). "
    "App aligns on keys, compares shared headings, computes Change/%Change for numeric fields, "
    "and flags Same/Different for text fields."
)
...
def guess_key_columns(df: pd.DataFrame, max_key_cols=5):
    """
    Heuristic picks categorical/low-cardinality columns as keys.
    """
    n = max(len(df), 1)
    cands = []
    for c in df.columns:
    ...
    if f1 and f2:
        ...
        if not shared:
            ...
            try:
                ...
                # Quick movers (numeric only)
                num = long_cmp[long_cmp["Type"] == "numeric"].copy()
                num = num.replace([np.inf, -np.inf], np.nan).dropna(subset=["% Change"])
                if not num.empty and used_keys:
                    label_key = st.selectbox("Chart label (key column)", used_keys, index=0)
                    topN = st.slider("Top N by % Change", 5, 50, 10)
                    by_field = st.selectbox("Filter Field (optional)", ["<All>"] + sorted(num["Field"].dropna().unique()))
                    plot_df = num if by_field == "<All>" else num[num["Field"] == by_field]
                    plot_df = plot_df.sort_values("% Change", ascending=False).head(topN)
                    if not plot_df.empty:
                        # lazy import plotly only when needed
                        import plotly.express as px
                        fig = px.bar(
                            plot_df, x=label_key, y="% Change", color="Field",
                            hover_data=used_keys + ["Field", "Last", "This", "Change"]
                        )
                        st.plotly_chart(fig, use_container_width=True)
                    else:
                        st.info("No plottable rows after filtering.")
...
# ---------- BATCH MODE ----------
    ...
    pair_regex = st.text_input(
        ...
        help="Files with same 'base' and different 'period' form a pair."
    )
